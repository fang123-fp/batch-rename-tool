...
## 2026-06-12 Manual Flow Refresh

### Goal
- 把首页流程改成“上传文件 -> 配置字段 -> 设置模板 -> 点击 `批量替换名称` 后再开始读取”
- 删除结果区里的“从内容重新读取”入口
- 用更简洁干净的视觉语言重做首页布局
...
- 已新增设计真相源：
  - `DESIGN.md`
  - `docs/2026-06-12-manual-flow-refresh.md`
- 首页交互已改成手动触发：
  - 上传文件后只排队显示，不再自动开始识别
  - 字段新增、删除、重命名后不再自动重读
  - 模板区新增唯一主 CTA：`批量替换名称`
  - 结果区移除了“从内容重新读取”按钮
- 结果表格已新增“预览名称”列，下载前可直接检查最终命名
- 页面样式已重做为更克制的单页工作台风格，并兼顾移动端单列布局
- 已继续把视觉收敛到更极简的 editorial 方向：
  - 主色改为低饱和陶土橙
  - 辅色加入鼠尾草绿和浅金色分区点缀
  - 分区与步骤色比上一版更明确，但仍保持低噪音
...
- `npm test`
- 本地页面人工检查：
  - 桌面端 `http://localhost:8123/` 首页布局通过
  - 移动端窄视口单列布局通过
  - 页面已确认模板区存在 `批量替换名称` 主按钮，结果区不再显示 `从内容重新读取`
